...
// Carries the current span context to work that may run on another thread.
class TraceParent {
 public:
...
// Starts a child span only when the current request is traced and ends it at scope exit.
class ScopedSpan {
 public:
  explicit ScopedSpan(std::string_view op_name);
  ~ScopedSpan();
